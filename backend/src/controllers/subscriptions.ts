...
subscriptionController.post('/list', async (req, res) => {
  const user = await handleAuth(req, res)
  if (!user) {
    return
  }
  ...
  const subscriptionByPublicId =
    await SubscriptionsUseCases.getUserListSubscriptions(req.body.userPublicIds)

  res.json(subscriptionByPublicId)
})

subscriptionController.post('/update', async (req, res) => {
