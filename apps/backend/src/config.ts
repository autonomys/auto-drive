...
export const config = {
  ...
  credits: {
    ...
    // Grace period (in minutes past expires_at) after which the cleanup sweep
    // expires a PENDING intent even though it carries a tx_hash.
    //
    // Reclamation only. It does NOT decide whether a payment is still credited —
    // that is SETTLE_GRACE_MS in core/users/intents.ts, and far shorter.
    //
    // A tx_hash exempts a row from the ordinary sweep because it means "actively
    // being watched": expiring it out from under a confirmation in flight would
    // race the watcher. A payment the watcher refused, and a transaction that
    // never confirms, are both standing counterexamples, and without a bound
    // those rows are reclaimed never.
    //
    // 24 hours by default: long enough that a genuinely pending transaction has
    // confirmed or been dropped by the mempool, so the sweep never takes a row
    // the watcher is still working on, and short enough that a stranded row is
    // reclaimed within a day rather than never.
